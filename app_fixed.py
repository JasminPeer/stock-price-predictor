...
df = yf.download(user_input, start=start, end=end, auto_adjust=True)  # auto_adjust avoids price adjustment warnings

# check if data exists
if df.empty:
    st.error(f"No data found for ticker '{user_input}'")
    st.stop()  # stop execution if ticker is invalid
